...
const actualizarEvento = async( req, res = response ) => {
    ...
    try {
        ...
        if ( !evento ) {
            return res.status(404).json({ ok: false, msg: 'Evento no existe por ese id' });
        }
...
const eliminarEvento = async( req, res = response ) => {
    ...
    try {
        ...
        if ( evento.user.toString() !== uid ) {
            return res.status(401).json({ ok: false, msg: 'No tiene privilegio de eliminar este evento' });
        }
